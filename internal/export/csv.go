...
var csvHeaders = []string{
	...
	"Bytes_Received",
	"Reverse_Sent_Mbps",
	"Reverse_Received_Mbps",
	"Reverse_Retransmits",
	"Reverse_Bytes_Sent",
	"Reverse_Bytes_Received",
...
func WriteCSV(path string, results []model.TestResult) error {
	...
	for _, r := range results {
		...
		row := []string{
			...
			fmt.Sprintf("%.2f", r.ReverseSentMbps()),
			fmt.Sprintf("%.2f", r.ReverseReceivedMbps()),
			strconv.Itoa(r.ReverseRetransmits),
			strconv.FormatInt(r.ReverseBytesSent, 10),
			strconv.FormatInt(r.ReverseBytesReceived, 10),
			baselineAvg,
			loadedAvg,
			loadedMax,
			r.Error,
